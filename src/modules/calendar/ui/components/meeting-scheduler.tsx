...
import { Video, Bot, Plus, Calendar, Clock, Zap } from "lucide-react";
import { meetingScheduleSchema, type MeetingSchedule } from "../../schema";
...
interface MeetingSchedulerProps {
  defaultDate?: Date;
  onSubmit: (data: MeetingSchedule) => Promise<void>;
  isLoading?: boolean;
}
...
export function MeetingScheduler({ defaultDate, onSubmit, isLoading = false }: MeetingSchedulerProps) {
  const [error, setError] = useState<string>("");
  const [meetingType, setMeetingType] = useState<'scheduled' | 'random'>('scheduled');
  const trpc = useTRPC();
  ...
  const handleFormSubmit = async (data: MeetingSchedule) => {
    try {
      setError("");
      
      if (meetingType === 'random') {
        // For random meetings, set start time to now
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    type="datetime-local"
                    value={formatDateTime(watch("scheduledStartTime"))}
      ...
      >
        {isLoading ? (
          <div className="flex items-center">
            ...
            {meetingType === 'scheduled' ? 'Scheduling...' : 'Starting...'}
          </div>
        ) : (
          <div className="flex items-center">
