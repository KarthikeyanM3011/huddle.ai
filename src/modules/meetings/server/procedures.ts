...
+import { meetings, agents, calendarEvents } from "@/db/schema";
...
+                        estimatedDuration: meetings.estimatedDuration,
...
+            // Create calendar event if it's a scheduled meeting
+            if (input.type === 'scheduled' && input.scheduledStartTime && input.estimatedDuration) {
...
+            // Update calendar event if meeting is scheduled
+            if (input.data.scheduledStartTime && input.data.estimatedDuration) {
+              const endTime = new Date(input.data.scheduledStartTime.getTime() + (input.data.estimatedDuration * 60 * 1000));
+              
+              await db
+                .update(calendarEvents)
+                .set({
+                  title: input.data.name || existingMeeting[0].name,
+                  startTime: input.data.scheduledStartTime,
+                  endTime: endTime,
+                  updatedAt: new Date(),
+                })
+                .where(eq(calendarEvents.meetingId, input.id));
+            }
